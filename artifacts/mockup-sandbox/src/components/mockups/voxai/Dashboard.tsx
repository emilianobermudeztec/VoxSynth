...
const translations = {
  EN: {
    ...
    aiPredictionDependency: "Temperature",
    context: "Context",
  ...
  ES: {
    ...
    aiPredictionDependency: "Temperatura",
    context: "Contexto",
...
export function Dashboard() {
  ...
  const openFlagModal = (id: number) => {
    setPendingFlagMsgId(id);
    setFlagReason('');
    setFlagCategory('');
    setFlagReasonModalOpen(true);
  };
  ...
  const submitFlagWithReason = () => {
    if (pendingFlagMsgId !== null) {
      setFlaggedMessages(prev => {
        ...
        next.add(pendingFlagMsgId);
        return next;
      });
      setShowRlhfNotification(true);
      setTimeout(() => setShowRlhfNotification(false), 4500);
    }
    setFlagReasonModalOpen(false);
    setPendingFlagMsgId(null);
  };
    ...
    <div style={{ minHeight: '100vh', background: colors.bg, color: colors.text, fontFamily: 'system-ui, -apple-system, sans-serif' }} className="flex flex-col overflow-hidden relative">
      ...
      <header className="h-16 border-b flex items-center justify-between px-6 shrink-0" style={{ backgroundColor: colors.cardBg, borderColor: colors.border }}>
        <div className="flex items-center gap-3">
          <img
            src={isDark ? voxsynthLogoDark : voxsynthLogo}
            alt="VoxSynth Lab"
            className="h-12 object-contain"
          />
          <span className="text-xs font-semibold italic tracking-wide" style={{ color: colors.secondary }}>
            {t.appSubtitle}
          </span>
        </div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-xl shadow-2xl flex flex-col border" style={{ backgroundColor: colors.cardBg, borderColor: colors.border }}>
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                <label className="text-[10px] font-bold uppercase tracking-widest block mb-2" style={{ color: colors.secondary }}>
                  Error Category
                </label>
                <div className="flex flex-wrap gap-2">
                  {['Incoherent', 'Tone Mismatch', 'Persona Drift', 'Fatigue Inconsistency', 'Language Error', 'Off-script', 'Other'].map(cat => (
                    <button
                      key={cat}
                      onClick={() => setFlagCategory(prev => prev === cat ? '' : cat)}
                      className="px-2.5 py-1 text-[10px] font-medium rounded border transition-all"
                      style={{
                        backgroundColor: flagCategory === cat ? colors.primary + '20' : colors.bg,
                        borderColor: flagCategory === cat ? colors.primary : colors.border,
                        color: flagCategory === cat ? colors.primary : colors.secondary
                      }}
                    >
                      {cat}
                    </button>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full text-xs rounded border p-3 resize-none focus:outline-none"
            ...
            <div className="flex gap-2 p-4 border-t" style={{ borderColor: colors.border }}>
              <button
                onClick={() => setFlagReasonModalOpen(false)}
